...
## What you need
- The **patched Cemu** build (the `mh3u-revival` Cemu fork — `Cemu_release.exe`).
- This server (Python 3.13+).
- Your **own legal MH3U dump** (you also play; nothing is distributed).
...
python server.py
```
It binds `0.0.0.0:1223` (auth) + `0.0.0.0:1224` (secure).
